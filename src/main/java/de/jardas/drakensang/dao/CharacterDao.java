...
public class CharacterDao {
    private static final org.slf4j.Logger LOG =
        org.slf4j.LoggerFactory.getLogger(CharacterDao.class);
    ...
    private static int getNumberOfCharacters() {
        final String sql =
            "select count(*) from _Instance_PC where name not like '%fake%' and name not like 'loc%' and name not like '%cutscene%'";

        try {
            final PreparedStatement statement = SavegameDao.getConnection().prepareStatement(sql);
            final ResultSet result = statement.executeQuery();
            result.next();

            return result.getInt(1);
        } catch (SQLException e) {
            throw new DrakensangException("Error preparing or executing SQL statement " + sql +
                ": " + e, e);
        }
    }
    ...
    private static Set<Character> loadCharactersInternal(Progress progress)
        ...
        final String sql =
            "select * from _Instance_PC where name not like '%fake%' and name not like 'loc%' and name not like '%cutscene%' order by Name";
        ResultSet result;

        try {
            PreparedStatement statement = SavegameDao.getConnection().prepareStatement(sql);
        ...
        while (result.next()) {
            ...
            c.setMaxVelocity(DaoHelper.round(result.getDouble("MaxVelocity"), 4));

            final String groups =
                (result.getString("Groups") != null) ? result.getString("Groups") : "";
            c.setCurrentPartyMember(groups.contains("_Group_PlayerParty"));
            c.setPartyMember(c.isCurrentPartyMember() || groups.contains("GainXpGroup"));

            if (c.isPlayerCharacter()) {
                c.setHair(HairDao.valueOf(result.getString("HairSkin")));
                c.setFace(FaceDao.valueOf(result.getString("HeadSkin")));
                c.setCharSet(CharSetDao.valueOf(result.getString("CharacterSet")));
            }

            InventoryDao.loadInventory(c, "_Instance_", SavegameDao.getConnection());
    ...
    private static void save(Character character) throws SQLException {
        UpdateStatementBuilder builder = new UpdateStatementBuilder("_Instance_PC", "Guid = ?");
        ...
        if (character.isPlayerCharacter()) {
            ...
            builder.append("'HairSkin' = ?", character.getHair().getId());
            builder.append("'HeadSkin' = ?", character.getFace().getId());
            builder.append("'CharacterSet' = ?", character.getCharSet().getId());
        }
