...
public class CharacterInfoPanel extends JPanel {
	...
	private void addDerivedFields() {
		...
		panel.add(leBonus, new GridBagConstraints(2, row++, 1, 1, 0, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(3, 6, 3, 6), 0, 0));
		...
		panel.add(aeBonus, new GridBagConstraints(2, row++, 1, 1, 0, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(3, 6, 3, 6), 0, 0));
	...
	private void addArchetypeFields(int panelRow) {
		...
		if (character.isPlayerCharacter()) {
			...
			archetypePanel.add(pic, new GridBagConstraints(2, 0, 1, 5, 0, 0,
			...
			protected void valueChanged(Sex item)
				...
				if (character.getCharacterSet().getArchetype(item) == null) {
					String message = MessageFormat.format(
							Messages.get("error.sexAppearance.message"),
		...
		if (character.isPlayerCharacter()) {
			appearanceCombo = new EnumComboBox<CharacterSet>(CharacterSet
					.values(), character.getCharacterSet()) {
	...
	private void updateAppearances() {
		if (appearanceCombo != null) {
			appearanceCombo.replaceValues(CharacterSet.values(), character
					.getCharacterSet());
		}
	}
	...
	private void addInput(JComponent parent, String label, String infoLabel,
		...
		parent.add(input, new GridBagConstraints(1, row, 1, 1, 0, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(3, 6, 3, 6), 0, 0));
	}
